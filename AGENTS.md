...
| HTTP API、路由、DTO、OpenAPI元数据、接口响应字段 | `.agents/rules/api-contract.md` |
| Go后端、Controller、Middleware、Service、插件后端、WASM host service | `.agents/rules/backend-go.md` |
| SQL迁移、Seed/Mock数据、DAO生成、软删除、数据库时间字段 | `.agents/rules/database.md` |
...
命中架构设计、数据权限或插件目录结构规范但未读取对应规则文件时，不得开始实现、通过审查或归档。若确认无数据权限、缓存、插件或模块启停影响，必须在任务记录或审查结论中明确记录。
...
服务层实现要求细则的唯一事实来源为`.agents/rules/backend-go.md`。修改`internal/service/`、插件`backend/internal/service/`、服务接口、构造函数、文件命名、事务、数据库访问、定时任务或上下文传递前，必须读取并严格遵守该规则文件。
