...
#### Scenario: 不支持的数据库方言在集群初始化前失败

- **WHEN**配置文件`database.default.link`以`sqlite:`、`mysql:`或未知前缀开头
- **AND**配置文件声明任意`cluster.enabled`值
- **THEN**宿主在方言解析阶段启动失败
- **AND**不启动选举循环、租约续期或节点投影同步
- **AND**不通过单机降级、启动警告或配置覆盖继续运行
