...
- **BREAKING**：新增`pkg/frameworkcap`作为框架能力统一公开入口，`orgcap`、`tenantcap`等“宿主定义接口、插件提供实现”的能力不再迁移为`frameworkcap/org`、`frameworkcap/tenant`公开子组件；用户统一通过`frameworkcap.Org()`、`frameworkcap.Tenant()`等根组件方法获取能力实例，provider registry、fallback、delegation 和激活实现放入`frameworkcap/internal`。
...
- **BREAKING**：插件之间不得直接 import 其他插件的 provider、service、dao、entity 或任何`internal`实现；消费方插件只能通过`pluginservice.Services.Framework().<Capability>()`或动态插件等价 guest client 消费稳定能力。
- 新增 framework capability provider factory 注册模型，由插件入口声明 provider factory，宿主按插件安装、启用、版本、依赖、租户状态和生命周期激活 provider，禁止 provider 在路由注册期间直接写入全局注册表。
- 不新增顶层`capabilities`配置块，也不在`dependencies`下新增 framework capability 依赖子块；硬依赖继续使用既有`dependencies.plugins`声明具体 provider 插件和版本约束，依赖项只保留`id`与`version`，不再支持`required`、`install`等软依赖或自动安装策略字段；可选能力通过`frameworkcap`运行时可用性检查和降级表达。
...
- `framework-capability-registry`: 定义框架能力公开契约、provider factory facade、内部 provider 激活、消费服务、运行时可用性和插件间能力消费治理。
...
- `core-host-boundary-governance`: 明确宿主通过`frameworkcap`和`pluginservice`暴露可选框架能力，不直接依赖插件内部实现或业务存储。
...
- `plugin-dependency-management`: 明确 framework capability 消费不新增依赖配置块；硬依赖复用既有`dependencies.plugins`的`id`和`version`约束，可选能力通过运行时可用性降级，并移除`required`、`install`等依赖策略字段。
- `plugin-upgrade-governance`: 要求插件升级、启用、禁用和刷新时重新校验 framework capability provider、既有下游插件依赖和能力可用性，并触发 provider 激活状态更新。
- `source-upgrade-governance`: 明确旧`sourceupgrade`实现不再作为公共组件边界，源码插件升级治理归属插件运行时升级内部实现。

## Impact

- 影响宿主公共契约：`apps/lina-core/pkg/pluginservice`、新增`pkg/frameworkcap`公开契约入口、现有`pkg/orgcap`、`pkg/tenantcap`、`pkg/pluginhost`、`pkg/pluginbridge`、`pkg/plugindb`。
- 影响宿主内部实现：插件运行时、源码插件 registrar、动态插件 host service registry、插件资源扫描、插件数据库 host executor、源码插件升级治理和 provider 激活管理。
...
- 影响动态插件 manifest：动态插件需要通过`hostServices`声明访问`frameworkcap`消费服务的授权边界；如需要硬依赖某个 provider 插件，继续使用既有`dependencies.plugins`和版本约束，不能绕过`pluginservice`或直接依赖其他插件实现。
- 数据权限影响：能力消费服务涉及用户、组织、租户、数据范围、候选项或批量信息时，必须在契约中定义数据权限边界，并禁止向插件泄漏底层`DAO`、`DO`、`Entity`、`*gdb.Model`或内部查询状态。
- 缓存一致性影响：framework capability provider 激活状态、插件 enabled snapshot、runtime revision 和能力可用性缓存属于关键运行时状态，集群模式下必须通过既有插件运行时修订、事件或共享缓存保持一致。
