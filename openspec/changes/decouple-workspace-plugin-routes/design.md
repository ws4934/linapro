...
1. 默认管理工作台使用可配置的非根 `basePath`，默认值为 `/admin`。

   本项目是全新项目，不需要为根路径管理工作台保留兼容模式。配置校验必须拒绝 `/`、空值、通配符、相对路径和宿主保留命名空间。工作台 `basePath` 以启动期配置为权威，后端 fallback、前端 router base、构建或启动配置、开发代理和 E2E 访问地址必须使用同一值；该配置不作为运行期热修改项处理，修改后需要重新启动对应服务并重新加载前端资源。备选方案是保留 `/` 并新增插件公开路由白名单，但该方案仍让工作台 fallback 持有根路径优先权，插件公开路由需要在宿主中持续登记例外，不符合源码插件闭环维护的目标。因此选择把工作台移到明确的非根入口路径。
...
9. 前端路由 base 与 API base 分离。
   ...
   管理工作台页面 URL 受 `workspace.basePath` 影响，宿主控制面 API 仍使用 `/api/v1`，源码插件和动态插件 API base 均使用 `/x/{plugin-id}/api/v1`。前端启动、登录跳转、刷新、E2E base URL、插件前端 API helper 和开发代理需要显式处理这些 base path，不能假设工作台部署在 `/`，也不能把宿主 `/api/v1` 当成任何插件 API 的默认 base。开发模式下以后端公开地址作为统一访问入口，后端在工作台 `basePath` 下代理或转发到 Vite dev server；开发者访问 `/admin` 时进入管理工作台，访问 `/` 或插件公开路由时仍到达后端源码插件路由，Vite 不得以根路径 SPA fallback 吞掉这些请求。
...
1. 新增工作台 basePath 配置与校验，默认值为 `/admin`，并拒绝 `/`、空值、通配符、相对路径和宿主保留命名空间。
